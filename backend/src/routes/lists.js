...
// Update active list
router.put('/active', auth_1.authenticate, async (req, res) => {
    try {
...
router.post('/items', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const existingItems = clearExisting ? [] : (list.items || []);
...
// IMPORTANT: This must be defined BEFORE /:id routes
router.delete('/items', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const normalizedNamesToDelete = itemNames.map((name) => name.toLowerCase().trim());
        // Filter out items that match the names to delete
        const remainingItems = existingItems.filter((item) => {
            const normalizedItemName = item.name.toLowerCase().trim();
            return !normalizedNamesToDelete.some((nameToDelete) => normalizedItemName === nameToDelete);
        });
